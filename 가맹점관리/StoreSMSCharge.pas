...
begin
  ...
  Result := ExecQuery(TempSQL, [], true, jsonSMSDBURL);
  if Result then
  begin
...
begin
  OpenQuery('select a.CD_CUSTOMER, '
           ...
           +' order by a.SEQ ',
           [DtoS(ConditionToolBarFromDateEdit.Date)],false,jsonSMSDBURL);
...
begin
  ...
  begin
    ...
    begin
      ...
      OpenQuery('select a.CD_CUSTOMER, '
               +'       a.NM_CUSTOMER, '
               +'       case a.DS_CUSTOMER when ''O'' then ''CloudOrange'' when ''E'' then ''ERP'' when ''M'' then ''Mart'' when ''T'' then ''Themark'' when ''S'' then ''Sauna'' end DS_PROGRAM, '
               ...
               +'   and ((a.DS_CUSTOMER = ''O'' and Length(a.CD_CUSTOMER) = 8) or a.DS_CUSTOMER in (''E'',''M'',''T'',''S'')) '
               +'   and a.DS_STATUS = ''0'' '
               +'union all '
               +'select a.CD_COMPANY, '
               +'       a.NM_COMPANY, '
               +'       case a.DS_COMPANY when ''OF'' then ''CloudOrange'' when ''EF'' then ''ERP'' end DS_PROGRAM, '
               +'       ConCat(a.CD_PARTNER,''-'',b.NM_PARTNER) as PARTNER '
               +'  from COMPANY a left outer join '
               +'       PARTNER  b on b.CD_PARTNER = a.CD_PARTNER '
               +' where (a.CD_COMPANY =:P0 or a.NM_COMPANY  like ConCat(''%'',:P0,''%'')) '
               +'   and  a.DS_COMPANY in (''OF'',''EF'') '
               +'   and a.YN_USE = ''Y'' '
               +'order by 1',
  ...
  with THelpForm.Create(Self) do
    try
      ...
      SQLText    := 'select a.CD_CUSTOMER, '
                   ...
                   +'  from COMPANY a left outer join '
                   +'       PARTNER  b on b.CD_PARTNER = a.CD_PARTNER '
                   +' where (a.CD_COMPANY =:P2 or a.NM_COMPANY  like ConCat(''%'',:P2,''%'')) '
                   +'   and  a.DS_COMPANY in (''OF'',''EF'') '
                   +'   and a.YN_USE = ''Y'' '
                   +'order by 1';
      SearchText := aSearch;
