inherited LetsOrderSaleShowForm: TLetsOrderSaleShowForm
  Caption = #47131#52768#50724#45908' '#47588#52636' '#51312#54924
  ClientHeight = 797
  ClientWidth = 1348
  ExplicitWidth = 1364
  ExplicitHeight = 836
  PixelsPerInch = 96
  TextHeight = 17
  inherited ToolBarDockPanel: TAdvDockPanel
    Width = 1348
    ExplicitWidth = 1348
    inherited ConditionToolBar: TAdvToolBar
      inherited ConditionToolBarFromDateEdit: TcxDateEdit
        Visible = True
      end
      inherited ConditionToolBarDateLabel: TcxLabel
        Visible = True
        AnchorX = 35
        AnchorY = 16
      end
      inherited ConditionToolBarToDateEdit: TcxDateEdit
        Visible = True
      end
      inherited ConditionToolBarDateToLabel: TcxLabel
        Visible = True
        AnchorX = 166
        AnchorY = 16
      end
      inherited ConditionToolBarEditLabel: TcxLabel
        Visible = True
        AnchorX = 448
        AnchorY = 16
      end
      inherited ConditionToolBarEdit: TcxTextEdit
        Visible = True
      end
      inherited ConditionToolbarMonthRadioButton: TcxRadioButton
        Visible = True
      end
      inherited ConditionToolbarTodayRadioButton: TcxRadioButton
        Visible = True
      end
      inherited ConditionToolbarWeekRadioButton: TcxRadioButton
        Visible = True
      end
    end
  end
  inherited Grid: TcxGrid
    Width = 1342
    Height = 726
    RootLevelOptions.DetailTabsPosition = dtpTop
    ExplicitWidth = 1342
    ExplicitHeight = 726
    inherited GridTableView: TcxGridTableView
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewLetsOrderAmt
        end
        item
          Format = ',0'
          Kind = skCount
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewColumn5
        end
        item
          Format = ',0 '#44148
          Kind = skCount
          Column = GridTableViewStoreCode
        end>
      DataController.Data = {
        
        6378537472696E6756616C75655479706500000000}
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      Styles.OnGetContentStyle = GridTableViewStylesGetContentStyle
      object GridTableViewStoreCode: TcxGridColumn
        Caption = #47588#52636#51068#51088
        DataBinding.FieldName = 'YMD_SALE'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        FooterAlignmentHorz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 115
      end
      object GridTableViewSaleAmt: TcxGridColumn
        Caption = #54252#49828
        DataBinding.FieldName = 'NO_POS'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Width = 58
      end
      object GridTableViewOrderAmt: TcxGridColumn
        Caption = #50689#49688#51613
        DataBinding.FieldName = 'NO_RCP'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        FooterAlignmentHorz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 83
      end
      object GridTableViewDsTrd: TcxGridColumn
        Caption = #44396#48516
        DataBinding.FieldName = 'DS_TRD'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Width = 73
      end
      object GridTableViewLetsOrderAmt: TcxGridColumn
        Caption = #44208#51228#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_APPROVAL'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Width = 141
      end
      object GridTableViewColumn5: TcxGridColumn
        Caption = #52712#49548#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_CANCEL'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Width = 110
      end
      object GridTableViewColumn6: TcxGridColumn
        Caption = #44208#51228#44396#48516
        DataBinding.FieldName = 'TYPE_TRD'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
      end
      object GridTableViewColumn1: TcxGridColumn
        Caption = #52852#46300#48264#54840
        DataBinding.FieldName = 'NO_CARD'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 175
      end
      object GridTableViewColumn2: TcxGridColumn
        Caption = #52852#46300#49324
        DataBinding.FieldName = 'NM_CARDPL'
        HeaderAlignmentHorz = taCenter
        Width = 127
      end
      object GridTableViewColumn3: TcxGridColumn
        Caption = #49849#51064#48264#54840
        DataBinding.FieldName = 'NO_APPROVAL'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 141
      end
      object GridTableViewColumn4: TcxGridColumn
        Caption = #49849#51064#51068#49884
        DataBinding.FieldName = 'APPROVAL_DATE'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 130
      end
    end
    object GridBandedTableView: TcxGridBandedTableView [1]
      Tag = 99
      Navigator.Buttons.CustomButtons = <>
      OnCellDblClick = GridBandedTableViewCellDblClick
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewTotCnt
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewTotAmt
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewLetsOrderTotCnt
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewLetsOrderTotAmt
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewLetsOrderPayCnt
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewLetsOrderPayAmt
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewLetsOrderOrderCnt
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableViewLetsOrderOrderAmt
        end
        item
          Format = ',0'
          Kind = skCount
          Column = GridBandedTableViewStoreName
        end>
      DataController.Summary.SummaryGroups = <>
      OptionsSelection.CellSelect = False
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      OptionsView.Indicator = True
      OptionsView.IndicatorWidth = 15
      Styles.ContentEven = StyleEven
      Bands = <
        item
          Caption = #47588#51109#51221#48372
          Width = 394
        end
        item
          Caption = #47588#52636#44552#50529
          Width = 177
        end
        item
          Caption = #44208#51228'+'#51452#47928
          Width = 241
        end
        item
          Caption = #44208#51228
          Width = 228
        end
        item
          Caption = #51452#47928'('#49440#48520#47588#51109#51228#50808')'
          Width = 226
        end>
      object GridBandedTableViewSeq: TcxGridBandedColumn
        Caption = 'No'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 41
        Position.BandIndex = 0
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableViewPartnerName: TcxGridBandedColumn
        Caption = #54801#47141#49324
        DataBinding.FieldName = 'NM_PARTNER'
        HeaderAlignmentHorz = taCenter
        Width = 74
        Position.BandIndex = 0
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableViewStoreCode: TcxGridBandedColumn
        Caption = #47588#51109#53076#46300
        DataBinding.FieldName = 'CD_STORE'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 81
        Position.BandIndex = 0
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableViewStoreName: TcxGridBandedColumn
        Caption = #47588#51109#47749
        DataBinding.FieldName = 'NM_STORE'
        FooterAlignmentHorz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 121
        Position.BandIndex = 0
        Position.ColIndex = 3
        Position.RowIndex = 0
      end
      object GridBandedTableViewType: TcxGridBandedColumn
        Caption = #50976#54805
        DataBinding.FieldName = 'DS_LETSORDER'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Styles.OnGetContentStyle = GridBandedTableViewTypeStylesGetContentStyle
        Width = 59
        Position.BandIndex = 0
        Position.ColIndex = 4
        Position.RowIndex = 0
      end
      object GridBandedTableViewTableCount: TcxGridBandedColumn
        Caption = #53580#51060#48660#49688
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'CNT_TABLE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 0
        Position.ColIndex = 5
        Position.RowIndex = 0
      end
      object GridBandedTableViewTotCnt: TcxGridBandedColumn
        Caption = #44148#49688
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'CNT_TOT'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Styles.Content = StyleFontBlackBold
        Width = 55
        Position.BandIndex = 1
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableViewTotAmt: TcxGridBandedColumn
        Caption = #47588#52636#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_TOT'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Styles.Content = StyleFontBlackBold
        Width = 122
        Position.BandIndex = 1
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderTotCnt: TcxGridBandedColumn
        Caption = #44148#49688
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'CNT_LETSORDER'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 2
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderTotAmt: TcxGridBandedColumn
        Caption = #47131#52768#50724#45908
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_LETSORDER'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Width = 104
        Position.BandIndex = 2
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderRate: TcxGridBandedColumn
        Caption = #48708#50984
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'RATE_LETSORDER'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = '#0.##%'
        HeaderAlignmentHorz = taCenter
        Width = 95
        Position.BandIndex = 2
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderPayCnt: TcxGridBandedColumn
        Caption = #44148#49688
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'CNT_PAY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 3
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderPayAmt: TcxGridBandedColumn
        Caption = #47131#52768#50724#45908
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_PAY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 3
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderPayRate: TcxGridBandedColumn
        Caption = #48708#50984
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'RATE_PAY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = '#0.##%'
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 3
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderOrderCnt: TcxGridBandedColumn
        Caption = #44148#49688
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'CNT_ORDER'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 4
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderOrderAmt: TcxGridBandedColumn
        Caption = #47131#52768#50724#45908
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_ORDER'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 4
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableViewLetsOrderOrderRate: TcxGridBandedColumn
        Caption = #48708#50984
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'RATE_ORDER'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = '#0.##%'
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 4
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableViewJsonURL: TcxGridBandedColumn
        Tag = 99
        Visible = False
        Position.BandIndex = -1
        Position.ColIndex = -1
        Position.RowIndex = -1
      end
      object GridBandedTableViewHeadStoreCode: TcxGridBandedColumn
        Tag = 99
        Caption = #48376#49324#53076#46300
        DataBinding.FieldName = 'CD_HEAD'
        Visible = False
        Position.BandIndex = -1
        Position.ColIndex = -1
        Position.RowIndex = -1
      end
    end
    object GridTableView1: TcxGridTableView [2]
      Navigator.Buttons.CustomButtons = <>
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
    end
    inherited GridLevel: TcxGridLevel
      Caption = ' '#47588#51109#48324' '#47588#52636' '
      GridView = GridBandedTableView
    end
    object GridLevel1: TcxGridLevel
      Caption = ' '#44208#51228#45236#50669'  '
      GridView = GridTableView
    end
  end
  inherited EditStyleController: TcxEditStyleController
    Style.IsFontAssigned = True
    OnStyleChanged = nil
    PixelsPerInch = 96
  end
  inherited StyleRepository: TcxStyleRepository
    PixelsPerInch = 96
  end
  inherited ButtonToolBarPrintButtonImageList: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited dxComponentPrinter: TdxComponentPrinter
    inherited dxComponentPrinter1Link1: TdxGridReportLink
      ReportDocument.CreationDate = 44937.645099768520000000
      BuiltInReportLink = True
    end
  end
end
