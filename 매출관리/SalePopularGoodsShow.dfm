inherited SalePopularGoodsShowForm: TSalePopularGoodsShowForm
  Caption = #51064#44592'/'#48708#51064#44592' '#49345#54408' '#51312#54924
  ClientWidth = 990
  HelpFile = #54980#48169'/'#49345#54408#44288#47532'/'#51064#44592'_'#48708#51064#44592'_'#49345#54408#51312#54924'.htm'
  ExplicitWidth = 1006
  PixelsPerInch = 96
  TextHeight = 17
  inherited ToolBarDockPanel: TAdvDockPanel
    Width = 990
    ExplicitWidth = 990
    inherited ButtonToolBar: TAdvToolBar
      Left = 606
      Width = 381
      ExplicitLeft = 606
      ExplicitWidth = 381
      inherited ButtonToolBarSeparator2: TAdvToolBarSeparator
        Left = 294
        ExplicitLeft = 294
      end
      object ButtonToolBarSeparator3: TAdvToolBarSeparator [2]
        Left = 184
        Top = 2
        Width = 10
        Height = 23
        SeparatorStyle = ssBlank
        LineColor = clBtnShadow
      end
      inherited ButtonToolBarCloseButton: TAdvGlowButton
        Left = 304
        ExplicitLeft = 304
      end
      inherited ButtonToolBarPrintButton: TAdvGlowMenuButton
        Left = 194
        ExplicitLeft = 194
      end
      object ButtonToolBarDeleteButton: TAdvGlowButton
        Left = 98
        Top = 2
        Width = 86
        Height = 27
        AutoSize = True
        Caption = #49325#51228'(F4)'
        DisabledPicture.Data = {
          }
        FocusType = ftHot
        NotesFont.Charset = DEFAULT_CHARSET
        NotesFont.Color = clWindowText
        NotesFont.Height = -11
        NotesFont.Name = 'Tahoma'
        NotesFont.Style = []
        Picture.Data = {
          
          AE426082}
        TabOrder = 3
        OnClick = ButtonToolBarDeleteButtonClick
        Appearance.BorderColor = 14727579
        Appearance.BorderColorHot = 10079963
        Appearance.BorderColorDown = 4548219
        Appearance.BorderColorChecked = 4548219
        Appearance.Color = 15653832
        Appearance.ColorTo = 16178633
        Appearance.ColorChecked = 11918331
        Appearance.ColorCheckedTo = 7915518
        Appearance.ColorDisabled = clNone
        Appearance.ColorDisabledTo = clNone
        Appearance.ColorDown = 7778289
        Appearance.ColorDownTo = 4296947
        Appearance.ColorHot = 15465983
        Appearance.ColorHotTo = 11332863
        Appearance.ColorMirror = 15586496
        Appearance.ColorMirrorTo = 16245200
        Appearance.ColorMirrorHot = 5888767
        Appearance.ColorMirrorHotTo = 10807807
        Appearance.ColorMirrorDown = 946929
        Appearance.ColorMirrorDownTo = 5021693
        Appearance.ColorMirrorChecked = 10480637
        Appearance.ColorMirrorCheckedTo = 5682430
        Appearance.ColorMirrorDisabled = clNone
        Appearance.ColorMirrorDisabledTo = clNone
        Appearance.GradientHot = ggVertical
        Appearance.GradientMirrorHot = ggVertical
        Appearance.GradientDown = ggVertical
        Appearance.GradientMirrorDown = ggVertical
        Appearance.GradientChecked = ggVertical
        Appearance.SystemFont = False
        Enabled = False
      end
    end
    inherited ConditionToolBar: TAdvToolBar
      Width = 939
      ExplicitWidth = 939
      inherited ConditionToolBarFromDateEdit: TcxDateEdit
        Left = 37
        Visible = True
        ExplicitLeft = 37
      end
      inherited ConditionToolBarDateLabel: TcxLabel
        Caption = #44592#44036
        Visible = True
        ExplicitWidth = 35
        Width = 35
        AnchorX = 20
        AnchorY = 16
      end
      inherited ConditionToolBarToDateEdit: TcxDateEdit
        Left = 145
        Visible = True
        ExplicitLeft = 145
      end
      inherited ConditionToolBarDateToLabel: TcxLabel
        Left = 127
        Visible = True
        ExplicitLeft = 127
        AnchorX = 136
        AnchorY = 16
      end
      inherited ConditionToolBarEditLabel: TcxLabel
        Left = 460
        Caption = #50948' '#44620#51648
        Visible = True
        ExplicitLeft = 460
        ExplicitWidth = 50
        Width = 50
        AnchorX = 485
        AnchorY = 16
      end
      inherited ConditionToolBarComboBox: TcxComboBox
        Left = 235
        Properties.Items.Strings = (
          #51064#44592' '#49345#54408
          #48708#51064#44592' '#49345#54408)
        Properties.OnChange = ConditionToolBarComboBoxPropertiesChange
        Text = #51064#44592' '#49345#54408
        Visible = True
        ExplicitLeft = 235
        ExplicitWidth = 95
        Width = 95
      end
      inherited ConditionToolBarEdit: TcxTextEdit
        Left = 430
        Properties.Alignment.Horz = taCenter
        Properties.MaxLength = 3
        Text = '50'
        Visible = True
        OnKeyDown = ConditionToolBarEditKeyDown
        ExplicitLeft = 430
        ExplicitWidth = 30
        Width = 30
      end
      inherited ConditionToolbarMonthRadioButton: TcxRadioButton
        Left = 510
        TabOrder = 11
        ExplicitLeft = 510
      end
      inherited ConditionToolbarTodayRadioButton: TcxRadioButton
        Left = 330
        TabOrder = 12
        ExplicitLeft = 330
      end
      inherited ConditionToolbarWeekRadioButton: TcxRadioButton
        Left = 380
        TabOrder = 13
        ExplicitLeft = 380
      end
      object ConditionToolBarPluCheckBox: TcxCheckBox
        Left = 560
        Top = 2
        Caption = 'PLU'#51228#50808
        ParentFont = False
        Style.StyleController = EditStyleController
        TabOrder = 7
        Width = 79
      end
      object ConditionToolBarEventCheckBox: TcxCheckBox
        Left = 639
        Top = 2
        Caption = #54665#49324#51228#50808
        ParentFont = False
        Style.StyleController = EditStyleController
        TabOrder = 8
        Width = 82
      end
      object ConditionToolBarOpenCheckBox: TcxCheckBox
        Left = 721
        Top = 2
        Caption = #50724#54536#45800#44032#51228#50808
        ParentFont = False
        Style.StyleController = EditStyleController
        TabOrder = 10
        Width = 105
      end
      object ConditionToolBarScaleCheckBox: TcxCheckBox
        Left = 826
        Top = 2
        Caption = #51200#50872#49345#54408#51228#50808
        ParentFont = False
        Style.StyleController = EditStyleController
        TabOrder = 9
        Width = 109
      end
    end
  end
  inherited Grid: TcxGrid
    Width = 984
    PopupMenu = GridPopup
    ExplicitWidth = 984
    inherited GridTableView: TcxGridTableView
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'#44148
          Kind = skCount
          Column = GridTableViewGoodsCode
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewSaleQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewSaleAmt
        end>
      DataController.Data = {
        }
      OptionsData.Editing = True
      OptionsSelection.CellSelect = True
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      object GridTableViewCheck: TcxGridColumn
        Tag = 98
        PropertiesClassName = 'TcxCheckBoxProperties'
        Properties.Alignment = taCenter
        Properties.NullStyle = nssUnchecked
        Properties.OnChange = GridTableViewCheckPropertiesChange
        Options.Filtering = False
        Options.Sorting = False
        Width = 25
        OnHeaderClick = GridTableViewCheckHeaderClick
      end
      object GridTableViewGoodsCode: TcxGridColumn
        Caption = #49345#54408#53076#46300
        DataBinding.FieldName = 'CD_GOODS'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 105
      end
      object GridTableViewGoodsName: TcxGridColumn
        Caption = #49345#54408#51060#47492
        DataBinding.FieldName = 'NM_GOODS'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 180
      end
      object GridTableViewGoodsSpec: TcxGridColumn
        Caption = #44508#44201
        DataBinding.FieldName = 'NM_SPEC'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 100
      end
      object GridTableViewSaleQty: TcxGridColumn
        Caption = #54032#47588#49688#47049
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SALE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableViewSaleAmt: TcxGridColumn
        Caption = #54032#47588#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_SALE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 90
      end
      object GridTableViewStockQty: TcxGridColumn
        Caption = #54788#51116#44256
        DataBinding.FieldName = 'QTY_STOCK'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableViewCompanyName: TcxGridColumn
        Caption = #44144#47000#52376
        DataBinding.FieldName = 'NM_TRDPL'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 150
      end
      object GridTableViewSaleDate: TcxGridColumn
        Caption = #52572#51333#54032#47588#51068#51088
        DataBinding.FieldName = 'YMD_SALE'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Options.Focusing = False
        Width = 103
      end
      object GridTableViewBuyDate: TcxGridColumn
        Caption = #52572#51333#47588#51077#51068#51088
        DataBinding.FieldName = 'YMD_BUY'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Options.Focusing = False
        Width = 105
      end
    end
  end
  inherited EditStyleController: TcxEditStyleController
    Style.IsFontAssigned = True
    PixelsPerInch = 96
  end
  inherited StyleRepository: TcxStyleRepository
    PixelsPerInch = 96
  end
  inherited ButtonToolBarPrintButtonImageList: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited dxComponentPrinter: TdxComponentPrinter
    inherited dxComponentPrinter1Link1: TdxGridReportLink
      PrinterPage.PageHeader.CenterTitle.Strings = (
        #51064#44592'/'#48708#51064#44592' '#49345#54408' '#51312#54924
        '')
      ReportDocument.CreationDate = 40423.441626157410000000
      BuiltInReportLink = True
    end
  end
  object GridPopup: TPopupMenu
    Left = 20
    Top = 120
    object GridPopupSelectAll: TMenuItem
      Caption = #47784#46160' '#49440#53469'(&A)'
      ShortCut = 16449
      OnClick = GridPopupSelectAllClick
    end
    object GridPopupDeselectAll: TMenuItem
      Caption = #47784#46160' '#49440#53469' '#52712#49548'(&C)'
      OnClick = GridPopupDeselectAllClick
    end
    object GridPopupInverseSelect: TMenuItem
      Caption = #49440#53469' '#50689#50669' '#48152#51204'(&I)'
      ShortCut = 16457
      OnClick = GridPopupInverseSelectClick
    end
  end
end
