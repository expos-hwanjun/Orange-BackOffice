...
type
  TMemberVatInvoicePopupForm = class(TForm)
    GridPanel: TPanel;
    Grid: TcxGrid;
...
var
  MemberVatInvoicePopupForm: TMemberVatInvoicePopupForm;
...
procedure TMemberVatInvoicePopupForm.FormCreate(Sender: TObject);
var vIndex :Integer;
begin
...
procedure TMemberVatInvoicePopupForm.FormKeyDown(Sender: TObject;
...
//------------------------------------------------------------------------------
// 현재 활성화 된 컨트롤 리턴
function TMemberVatInvoicePopupForm.GetActiveControl: TWinControl;
begin
...
procedure TMemberVatInvoicePopupForm.SearchButtonClick(Sender: TObject);
begin
  try
    ...
    MainPanel.Enabled := DM.ReadQuery(CompanyGridTableView);
...
procedure TMemberVatInvoicePopupForm.CloseButtonClick(Sender: TObject);
begin
  Close;
end;

procedure TMemberVatInvoicePopupForm.CompanyGridTableViewFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
...
begin
  ...
  try
    ...
    DM.ReadQuery(SaleGridTableView);
  finally
    DM.FinishQuery;
  end;
    ...
    DM.OpenQuery('select dbo.StoD(YMD_OCCUR) as YMD_OCCUR, '
                ...
                +'order by YMD_OCCUR, NO_ACCT ',
                 [StoreCode,
                  ...
                  DtoS(ConditionToolBarToDateEdit.Date),
                  AFocusedRecord.Values[0]]);
    DM.ReadQuery(AcctGridTableView);
  finally
    DM.FinishQuery;
  end;
end;
...
//확인버튼 클릭 시
procedure TMemberVatInvoicePopupForm.ConfirmButtonClick(Sender: TObject);
var vIndex :Integer;
begin
  if TotalAmtEdit.Value = 0 then
  begin
    MsgBox('발행 할 금액이 없습니다');
    Exit;
  end;
...
procedure TMemberVatInvoicePopupForm.SaleGridTableViewCheckHeaderClick(Sender: TObject);
var vIndex :Integer;
begin
...
procedure TMemberVatInvoicePopupForm.AcctGridTableViewCheckHeaderClick(
  Sender: TObject);
var vIndex :Integer;
begin
...
procedure TMemberVatInvoicePopupForm.SaleGridTableViewCheckPropertiesEditValueChanged(
  Sender: TObject);
var vIndex :Integer;
begin
...
procedure TMemberVatInvoicePopupForm.AcctGridTableViewCheckPropertiesEditValueChanged(
  Sender: TObject);
var vIndex :Integer;
begin
