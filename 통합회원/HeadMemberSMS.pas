...
type
  THeadMemberSMSForm = class(TInheritEditForm)
    SMSPanel: TPanel;
    ...
    CheckButton: TButton;
    MemberGridViewTotSaleAmt: TcxGridColumn;
...
var
  HeadMemberSMSForm: THeadMemberSMSForm;
...
//------------------------------------------------------------------------------
// 폼을 만들 때
procedure THeadMemberSMSForm.FormCreate(Sender: TObject);
begin
  inherited;
  DefaultDate := ddNextWeek;
end;
procedure THeadMemberSMSForm.FormResize(Sender: TObject);
begin
  inherited;
...
//------------------------------------------------------------------------------
// 폼을 보여줄 때
procedure THeadMemberSMSForm.FormShow(Sender: TObject);
var
  vCode: PStrPointer;
begin
  inherited;
...
//------------------------------------------------------------------------------
// 입력창을 수정할 때
procedure THeadMemberSMSForm.EditPropertiesChange(Sender: TObject);
var vTemp :String;
begin
  inherited;
...
//------------------------------------------------------------------------------
// 이모티콘 레이블을 클릭할 때
procedure THeadMemberSMSForm.SMSSymbolPagerSymbolLabelClick(Sender: TObject);
var
  vStr: string;
begin
  inherited;
...
procedure THeadMemberSMSForm.TelNoMemoKeyPress(Sender: TObject; var Key: Char);
var vIndex :Integer;
begin
  inherited;
  if Key <> #13 then Exit;
...
procedure THeadMemberSMSForm.TransTypeButtonClick(Sender: TObject);
begin
  inherited;
end;
...
//------------------------------------------------------------------------------
// 메시지함 콤보박스를 변경할 때
procedure THeadMemberSMSForm.SMSSymbolPagerMessageComboBoxPropertiesChange(Sender: TObject);
begin
  inherited;
...
//------------------------------------------------------------------------------
// 메시지함 입력 버튼
procedure THeadMemberSMSForm.SMSSymbolPagerMessageButtonClick(Sender: TObject);
begin
  inherited;
...
procedure THeadMemberSMSForm.SetDupPhoneNo;
var vIndex :Integer;
begin
  try
...
procedure THeadMemberSMSForm.SMSImmediateRadioButtonClick(Sender: TObject);
begin
  inherited;
...
procedure THeadMemberSMSForm.SMSMessageMemoEnter(Sender: TObject);
begin
  inherited;
  KeyPreview := false;
end;

procedure THeadMemberSMSForm.SMSMessageMemoExit(Sender: TObject);
begin
  inherited;
  KeyPreview := true;
end;
...
//------------------------------------------------------------------------------
// 보내는 전화번호를 빠져나갈 때
procedure THeadMemberSMSForm.SMSPhoneEditExit(Sender: TObject);
begin
  inherited;
...
//------------------------------------------------------------------------------
// 전송 시간 라디오 버튼을 선택할 때
procedure THeadMemberSMSForm.SMSRadioButtonClick(Sender: TObject);
begin
  inherited;
...
//------------------------------------------------------------------------------
// 조회 결과 탭을 변경할 때
procedure THeadMemberSMSForm.ReservationGridViewDataControllerSummaryAfterSummary(
  ASender: TcxDataSummary);
var
...
procedure THeadMemberSMSForm.ReservationGridViewSendHeaderClick(Sender: TObject);
var vIndex :Integer;
begin
  inherited;
...
procedure THeadMemberSMSForm.ReservationGridViewSendPropertiesEditValueChanged(
  Sender: TObject);
begin
  inherited;
...
procedure THeadMemberSMSForm.MemberAddButtonClick(Sender: TObject);
begin
  inherited;
  MemberGridView.DataController.BeginUpdate;
...
procedure THeadMemberSMSForm.MemberGridViewDataControllerFilterChanged(
  Sender: TObject);
var vIndex :Integer;
begin
  try
...
procedure THeadMemberSMSForm.MemberGridViewDataControllerSummaryAfterSummary(
  ASender: TcxDataSummary);
var
  vIndex, vCount: Integer;
begin
...
procedure THeadMemberSMSForm.MemberGridViewFocusedRecordChanged(
...
procedure THeadMemberSMSForm.MemberGridViewMobileNoColumnPropertiesValidate(
...
procedure THeadMemberSMSForm.MemberGridViewSendColumnHeaderClick(
  Sender: TObject);
var vIndex :Integer;
begin
  inherited;
...
procedure THeadMemberSMSForm.MemberGridViewSendColumnPropertiesEditValueChanged(
  Sender: TObject);
begin
  inherited;
...
procedure THeadMemberSMSForm.MemberGridViewStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
begin
  inherited;
...
//------------------------------------------------------------------------------
// SMS 충전 정보 읽기
function THeadMemberSMSForm.GetCustomerSmsInfo(aMsg:Boolean): Boolean;
var
  vResultStr: String;
...
begin
  ...
  Domain := 'http://전단.kr';
  try
    GetServiceSoap.smsCount2(HeadStoreCode,
                             vResult,
                             vChargeAmt,
                             vRemainAmt,
                             vSmsUseQty,
...
function THeadMemberSMSForm.GetFilteredSort(aGrid: TcxGridTableView): Boolean;
var vIndex :Integer;
begin
  Result := false;
...
procedure THeadMemberSMSForm.HelpCloseButtonClick(Sender: TObject);
begin
  inherited;
...
procedure THeadMemberSMSForm.LmsRadioButtonClick(Sender: TObject);
var vTemp :String;
begin
  inherited;
...
procedure THeadMemberSMSForm.MemberNameButtonClick(Sender: TObject);
...
procedure THeadMemberSMSForm.NoticePanelClose(Sender: TObject);
begin
  inherited;
  ToolBarDockPanel.Enabled := true;
  SMSPanel.Enabled         := true;
...
procedure THeadMemberSMSForm.NoticeRadioButtonClick(Sender: TObject);
begin
  inherited;
...
//------------------------------------------------------------------------------
// 조회 버튼
function THeadMemberSMSForm.DoSearch: Boolean;
  ...
  case TransTypeComboBox.ItemIndex of
    ...
    begin
      ...
      if (ConditionVisitBeforeDateEdit.Date > 0) then
        vWhere[3] := Format('and t.YMD_VISIT <= ''%s'' ',[DtoS(ConditionVisitBeforeDateEdit.Date)]);

      if (ConditionVisitAfterDateEdit.Date > 0) then
        vWhere[4] := Format('and t.YMD_VISIT >= ''%s'' ',[DtoS(ConditionVisitAfterDateEdit.Date)]);

      if ConditionPointCheckBox.Checked then
        vWhere[5] := Format('and g.TOTAL_POINT Between %s and %s ', [FormatFloat('#0',ConditionFromPointEdit.Value), FormatFloat('#0',ConditionToPointEdit.Value)]);

      if (vWhere[0] <> EmptyStr) and ConditionSaleAmtCheckBox.Checked then
        vWhere[6] := Format('and t.AMT_SALE Between %s and %s ', [FormatFloat('#0',ConditionFromAmtEdit.Value), FormatFloat('#0',ConditionToAmtEdit.Value)])
      ...
      try
        ...
        OpenQuery('select   ''1'' as CHK, '
                 ...
                 +'         StoD(t.YMD_VISIT) as YMD_VISIT, '
                 +'         t.CNT_VISIT as CNT_VISIT_TOT, '
                 ...
                 +'         MS_CODE   f on f.CD_HEAD  = a.CD_HEAD '
                 ...
                 +'           from MS_MEMBER_ETC '
                 +'          where CD_HEAD  =:P0 '
                 +'          group by CD_MEMBER) as t on t.CD_MEMBER = a.CD_MEMBER'
                 +vWhere[0]
                 +vWhere[7]
                 +vWhere[8]
                 +'   where a.CD_HEAD  =:P0 '
...
//------------------------------------------------------------------------------
// 저장 버튼
procedure THeadMemberSMSForm.AdDesignMakeButtonClick(Sender: TObject);
var vVersion :String;
begin
  inherited;
...
procedure THeadMemberSMSForm.AdRadioButtonClick(Sender: TObject);
begin
  inherited;
...
procedure THeadMemberSMSForm.AgreeCheckBoxPropertiesChange(Sender: TObject);
begin
  inherited;
  if AgreeCheckBox.Checked then
  begin
...
procedure THeadMemberSMSForm.Button1Click(Sender: TObject);
...
procedure THeadMemberSMSForm.ButtonToolBarSearchButtonClick(Sender: TObject);
...
procedure THeadMemberSMSForm.ChargeSearchButtonClick(Sender: TObject);
begin
  inherited;
  GetCustomerSmsInfo(false);
end;

procedure THeadMemberSMSForm.CheckButtonClick(Sender: TObject);
var vIndex :Integer;
begin
  inherited;
  try
...
procedure THeadMemberSMSForm.ConditionToolBarHelpButtonClick(Sender: TObject);
begin
  inherited;
  ToolBarDockPanel.Enabled := false;
  SMSPanel.Enabled         := false;
...
procedure THeadMemberSMSForm.cxHyperLinkEdit1Click(Sender: TObject);
begin
  inherited;
...
procedure THeadMemberSMSForm.cxHyperLinkEdit3Click(Sender: TObject);
var vURL :String;
begin
  inherited;
...
function THeadMemberSMSForm.DoSave: Boolean;
