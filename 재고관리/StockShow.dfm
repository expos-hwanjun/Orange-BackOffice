inherited StockShowForm: TStockShowForm
  Left = 300
  Top = 120
  ActiveControl = ConditionToolBarEdit
  Caption = #51116#44256' '#51312#54924
  ClientHeight = 602
  ClientWidth = 964
  HelpFile = #54980#48169'/'#51116#44256#44288#47532'/'#51116#44256#51312#54924'.htm'
  ExplicitWidth = 980
  ExplicitHeight = 641
  PixelsPerInch = 96
  TextHeight = 17
  inherited ToolBarDockPanel: TAdvDockPanel
    Width = 964
    ExplicitWidth = 964
    inherited ButtonToolBar: TAdvToolBar
      Left = 676
      ExplicitLeft = 676
    end
    inherited ConditionToolBar: TAdvToolBar
      inherited ConditionToolBarFromDateEdit: TcxDateEdit
        Visible = True
      end
      inherited ConditionToolBarDateLabel: TcxLabel
        Caption = #44592#51456#51068#51088
        Visible = True
        AnchorX = 35
        AnchorY = 16
      end
      inherited ConditionToolBarDateToLabel: TcxLabel
        AnchorX = 166
        AnchorY = 16
      end
      inherited ConditionToolBarEditLabel: TcxLabel
        ParentShowHint = False
        ShowHint = True
        Visible = True
        ExplicitHeight = 25
        Height = 25
        AnchorX = 448
        AnchorY = 15
      end
      inherited ConditionToolBarEdit: TcxTextEdit
        Hint = #49345#54408#51060#47492', '#49345#54408#53076#46300', '#44144#47000#52376#51060#47492' '#46321'...'
        ParentShowHint = False
        ShowHint = True
        Visible = True
      end
    end
  end
  inherited Grid: TcxGrid
    Width = 958
    Height = 531
    RootLevelOptions.DetailTabsPosition = dtpTop
    OnActiveTabChanged = GridActiveTabChanged
    ExplicitWidth = 958
    ExplicitHeight = 531
    inherited GridTableView: TcxGridTableView
      OnCellDblClick = GridTableViewCellDblClick
      DataController.Summary.DefaultGroupSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Position = spFooter
          Column = GridTableViewSurveyQty
        end
        item
          Format = ',0'
          Kind = skSum
          Position = spFooter
        end>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'#44148
          Kind = skCount
          Column = GridTableViewGoodsCode
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewSurveyQty
        end
        item
          Format = ',0'
          Kind = skSum
        end>
      DataController.Data = {
        
        79706512000000546378537472696E6756616C75655479706500000000}
      OptionsSelection.CellSelect = True
      OptionsView.GroupByBox = False
      OptionsView.GroupFooters = gfAlwaysVisible
      Styles.ContentEven = StyleEven
      Styles.OnGetContentStyle = GridTableViewStylesGetContentStyle
      object GridTableViewCompany: TcxGridColumn
        Caption = #44144#47000#52376
        DataBinding.FieldName = 'NM_TRDPL'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Vert = taVCenter
        HeaderAlignmentHorz = taCenter
        Options.Focusing = False
        Width = 150
      end
      object GridTableViewGoodsCode: TcxGridColumn
        Caption = #49345#54408#53076#46300
        DataBinding.FieldName = 'CD_GOODS'
        PropertiesClassName = 'TcxTextEditProperties'
        Properties.ReadOnly = True
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Focusing = False
        Options.Grouping = False
        Options.Moving = False
        Width = 110
      end
      object GridTableViewGoodsName: TcxGridColumn
        Caption = #49345#54408#51060#47492
        DataBinding.FieldName = 'NM_GOODS'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Vert = taVCenter
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Focusing = False
        Options.Grouping = False
        Options.Moving = False
        Width = 200
      end
      object GridTableViewGoodsSpec: TcxGridColumn
        Caption = #44508#44201
        DataBinding.FieldName = 'NM_SPEC'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Vert = taVCenter
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Focusing = False
        Options.Grouping = False
        Options.Moving = False
        Width = 100
      end
      object GridTableViewStockQty: TcxGridColumn
        Caption = #51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Focusing = False
        Options.Grouping = False
        Options.Moving = False
        Width = 70
      end
      object GridTableViewSurveyQty: TcxGridColumn
        Caption = #47588#51077#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Focusing = False
        Options.Grouping = False
        Options.Moving = False
        Width = 100
      end
      object GridTableViewMenuType: TcxGridColumn
        Tag = 99
        Caption = #49345#54408#44396#48516
        DataBinding.FieldName = 'DS_MENU_TYPE'
        Visible = False
      end
    end
    object GridBandedTableView1: TcxGridBandedTableView [1]
      Tag = 99
      Navigator.Buttons.CustomButtons = <>
      OnCellDblClick = GridBandedTableView1CellDblClick
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1StockQtyBef
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1StockBuyAmtBef
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1StockSaleAmtBef
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1BuyQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1SaleQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1DisuseQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1SilsaQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1StockQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1StockBuyAmt
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView1Column1
        end>
      DataController.Summary.SummaryGroups = <>
      OptionsSelection.CellSelect = False
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      Bands = <
        item
          Caption = #44144#47000#52376
          Width = 210
        end
        item
          Caption = #51060#50900#51116#44256
          Width = 270
        end
        item
          Caption = #44592#44036#51116#44256
          Width = 420
        end
        item
          Caption = #54788#51116#44256
          Width = 270
        end>
      object GridBandedTableView1TrdName: TcxGridBandedColumn
        Caption = #44144#47000#52376#51060#47492
        DataBinding.FieldName = 'NM_TRDPL'
        HeaderAlignmentHorz = taCenter
        Options.Moving = False
        Width = 150
        Position.BandIndex = 0
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView1StockQtyBef: TcxGridBandedColumn
        Caption = #51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK_BEF'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Moving = False
        Width = 70
        Position.BandIndex = 1
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView1StockBuyAmtBef: TcxGridBandedColumn
        Caption = #47588#51077#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_BUY_BEF'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Moving = False
        Width = 100
        Position.BandIndex = 1
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView1StockSaleAmtBef: TcxGridBandedColumn
        Caption = #54032#47588#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_SALE_BEF'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Moving = False
        Width = 100
        Position.BandIndex = 1
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableView1BuyQty: TcxGridBandedColumn
        Caption = #47588#51077
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Moving = False
        Width = 85
        Position.BandIndex = 2
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView1SaleQty: TcxGridBandedColumn
        Caption = #47588#52636
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SALE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Moving = False
        Width = 88
        Position.BandIndex = 2
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableView1DisuseQty: TcxGridBandedColumn
        Caption = #54224#44592
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_DISUSE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 88
        Position.BandIndex = 2
        Position.ColIndex = 3
        Position.RowIndex = 0
      end
      object GridBandedTableView1SilsaQty: TcxGridBandedColumn
        Caption = #49892#49324#51312#51221
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SILSA'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 88
        Position.BandIndex = 2
        Position.ColIndex = 4
        Position.RowIndex = 0
      end
      object GridBandedTableView1StockQty: TcxGridBandedColumn
        Caption = #51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Moving = False
        Width = 70
        Position.BandIndex = 3
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView1StockBuyAmt: TcxGridBandedColumn
        Caption = #47588#51077#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Options.Moving = False
        Width = 100
        Position.BandIndex = 3
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView1TrdCode: TcxGridBandedColumn
        Tag = 99
        Caption = #44144#47000#52376#53076#46300
        DataBinding.FieldName = 'CD_TRDPL'
        Visible = False
        Position.BandIndex = 0
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView1Column1: TcxGridBandedColumn
        Caption = #48152#54408
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_RETURN'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Width = 71
        Position.BandIndex = 2
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
    end
    object GridBandedTableView2: TcxGridBandedTableView [2]
      Tag = 99
      Navigator.Buttons.CustomButtons = <>
      OnCellDblClick = GridBandedTableView2CellDblClick
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockQtyBef
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockBuyAmtBef
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockSaleAmtBef
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockBuy
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockSale
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockDisuse
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockSilsa
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockBuyAmt
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView2StockReturn
        end>
      DataController.Summary.SummaryGroups = <>
      OptionsSelection.CellSelect = False
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      Bands = <
        item
          Caption = #49345#54408#48516#47448
          Width = 200
        end
        item
          Caption = #51060#50900#51116#44256
          Width = 270
        end
        item
          Caption = #44592#44036#51116#44256
          Width = 420
        end
        item
          Caption = #54788#51116#44256
          Width = 270
        end>
      object GridBandedTableView2ClassName: TcxGridBandedColumn
        Caption = #49345#54408#48516#47448#51060#47492
        DataBinding.FieldName = 'NM_CLASS'
        HeaderAlignmentHorz = taCenter
        Position.BandIndex = 0
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockQtyBef: TcxGridBandedColumn
        Caption = #51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK_BEF'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 1
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockBuyAmtBef: TcxGridBandedColumn
        Caption = #47588#51077#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_BUY_BEF'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 100
        Position.BandIndex = 1
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockSaleAmtBef: TcxGridBandedColumn
        Caption = #54032#47588#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_SALE_BEF'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 100
        Position.BandIndex = 1
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockBuy: TcxGridBandedColumn
        Caption = #47588#51077
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockSale: TcxGridBandedColumn
        Caption = #47588#52636
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SALE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockDisuse: TcxGridBandedColumn
        Caption = #54224#44592
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_DISUSE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 3
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockSilsa: TcxGridBandedColumn
        Caption = #49892#49324#51312#51221
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SILSA'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 4
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockQty: TcxGridBandedColumn
        Caption = #51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 3
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockBuyAmt: TcxGridBandedColumn
        Caption = #47588#51077#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 100
        Position.BandIndex = 3
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView2ClassCode: TcxGridBandedColumn
        Tag = 99
        DataBinding.FieldName = 'CD_CLASS'
        Visible = False
        Position.BandIndex = 0
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView2StockReturn: TcxGridBandedColumn
        Caption = #48152#54408
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_RETURN'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.DisplayFormat = ',0.##'
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Position.BandIndex = 2
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
    end
    object GridBandedTableView3: TcxGridBandedTableView [3]
      Tag = 99
      Navigator.Buttons.CustomButtons = <>
      OnCellDblClick = GridBandedTableView3CellDblClick
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3StockQtyBef
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3BuyQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3SaleQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3DisuseQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3SilsaQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3StockQty
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3StockBuyAmt
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'#44148
          Kind = skCount
          Column = GridBandedTableView3GoodsCode
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridBandedTableView3Column1
        end>
      DataController.Summary.SummaryGroups = <>
      OptionsSelection.CellSelect = False
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      Styles.OnGetContentStyle = GridBandedTableView3StylesGetContentStyle
      Bands = <
        item
          Caption = #49345#54408#51221#48372
          Width = 410
        end
        item
          Caption = #51204#51116#44256
          Width = 70
        end
        item
          Caption = #44592#44036#51116#44256
          Width = 420
        end
        item
          Caption = #54788#51116#44256
          Width = 270
        end>
      object GridBandedTableView3GoodsCode: TcxGridBandedColumn
        Caption = #49345#54408#53076#46300
        DataBinding.FieldName = 'CD_GOODS'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 110
        Position.BandIndex = 0
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView3GoodsName: TcxGridBandedColumn
        Caption = #49345#54408#47749
        DataBinding.FieldName = 'NM_GOODS'
        FooterAlignmentHorz = taCenter
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 200
        Position.BandIndex = 0
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView3Spec: TcxGridBandedColumn
        Caption = #44508#44201
        DataBinding.FieldName = 'NM_SPEC'
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 100
        Position.BandIndex = 0
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableView3StockQtyBef: TcxGridBandedColumn
        Caption = #51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK_BEF'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 1
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView3BuyQty: TcxGridBandedColumn
        Caption = #47588#51077
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView3Column1: TcxGridBandedColumn
        Caption = #48152#54408
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_RETURN'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Position.BandIndex = 2
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
      object GridBandedTableView3SaleQty: TcxGridBandedColumn
        Caption = #47588#52636
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SALE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 2
        Position.RowIndex = 0
      end
      object GridBandedTableView3DisuseQty: TcxGridBandedColumn
        Caption = #54224#44592
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_DISUSE'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 3
        Position.RowIndex = 0
      end
      object GridBandedTableView3SilsaQty: TcxGridBandedColumn
        Caption = #49892#49324#51312#51221
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SILSA'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 2
        Position.ColIndex = 4
        Position.RowIndex = 0
      end
      object GridBandedTableView3StockQty: TcxGridBandedColumn
        Caption = #51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
        Position.BandIndex = 3
        Position.ColIndex = 0
        Position.RowIndex = 0
      end
      object GridBandedTableView3StockBuyAmt: TcxGridBandedColumn
        Caption = #47588#51077#44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 100
        Position.BandIndex = 3
        Position.ColIndex = 1
        Position.RowIndex = 0
      end
    end
    object GridTableView1: TcxGridTableView [4]
      Tag = 99
      Navigator.Buttons.CustomButtons = <>
      DataController.Filter.OnChanged = GridTableView1DataControllerFilterChanged
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Column = GridTableView1Qty
        end
        item
          Format = ',0 '#44148
          Kind = skCount
          Column = GridTableView1Date
        end>
      DataController.Summary.SummaryGroups = <>
      OptionsSelection.CellSelect = False
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      Styles.OnGetContentStyle = GridTableView1StylesGetContentStyle
      object GridTableView1Date: TcxGridColumn
        Caption = #51068#51088
        DataBinding.FieldName = 'YMD_BUY'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        FooterAlignmentHorz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 110
      end
      object GridTableView1Gubun: TcxGridColumn
        Caption = #44396#48516
        DataBinding.FieldName = 'DS_GUBUN'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Width = 70
      end
      object GridTableView1Qty: TcxGridColumn
        Caption = #49688#47049
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 86
      end
      object GridTableView1Price: TcxGridColumn
        Caption = #45800#44032
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'PR_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableView1Amount: TcxGridColumn
        Caption = #44552#50529
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'AMT_BUY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DisplayFormat = ',0'
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 100
      end
      object GridTableView1BuyTrd: TcxGridColumn
        Caption = #44144#47000#52376
        DataBinding.FieldName = 'NM_TRDPL'
        HeaderAlignmentHorz = taCenter
        Width = 150
      end
      object GridTableView1RcpNo: TcxGridColumn
        Caption = #51204#54364#48264#54840
        DataBinding.FieldName = 'NO_BUY'
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 100
      end
      object GridTableView1Seq: TcxGridColumn
        Tag = 99
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'SEQ'
        Visible = False
      end
    end
    inherited GridLevel: TcxGridLevel
      Caption = ' '#49345#54408#48324' '#51116#44256' '
    end
    object GridLevel1: TcxGridLevel
      Caption = ' '#44144#47000#52376#48324' '#51116#44256' '
      GridView = GridBandedTableView1
    end
    object GridLevel2: TcxGridLevel
      Caption = ' '#49345#54408#48516#47448#48324' '#51116#44256' '
      GridView = GridBandedTableView2
    end
    object GridLevel3: TcxGridLevel
      Caption = ' '#49345#54408#48324' '#49688#48520' '
      GridView = GridBandedTableView3
    end
    object GridLevel4: TcxGridLevel
      Caption = ' '#49345#54408#51109' '
      GridView = GridTableView1
    end
  end
  inherited EditStyleController: TcxEditStyleController
    Style.IsFontAssigned = True
    PixelsPerInch = 96
  end
  inherited StyleRepository: TcxStyleRepository
    PixelsPerInch = 96
  end
  inherited ButtonToolBarPrintButtonImageList: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited ButtonToolBarPrintButtonPopupMenu: TPopupMenu
    inherited ButtonToolBarPrintButtonPopupMenuReceipt: TMenuItem
      Visible = True
    end
  end
  inherited dxComponentPrinter: TdxComponentPrinter
    inherited dxComponentPrinter1Link1: TdxGridReportLink
      PrinterPage.PageHeader.CenterTitle.Strings = (
        #51116#44256#51312#54924)
      ReportDocument.CreationDate = 40423.444390000000000000
      OptionsView.Caption = True
      BuiltInReportLink = True
    end
  end
end
