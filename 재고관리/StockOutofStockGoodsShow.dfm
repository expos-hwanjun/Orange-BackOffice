inherited StockOutofStockGoodsShowForm: TStockOutofStockGoodsShowForm
  Left = 300
  Top = 120
  Caption = #54408#51208' '#50696#49345' '#49345#54408' '#51312#54924
  ClientHeight = 602
  ClientWidth = 964
  Ctl3D = False
  HelpFile = #54980#48169'/'#51116#44256#44288#47532'/'#54408#51208#50696#49345'_'#49345#54408#51312#54924'.htm'
  ExplicitTop = -41
  ExplicitWidth = 980
  ExplicitHeight = 640
  PixelsPerInch = 96
  TextHeight = 17
  inherited ToolBarDockPanel: TAdvDockPanel
    Width = 964
    ExplicitWidth = 964
    inherited ButtonToolBar: TAdvToolBar
      Left = 590
      Width = 371
      ExplicitLeft = 590
      ExplicitWidth = 371
      inherited ButtonToolBarSeparator2: TAdvToolBarSeparator
        Left = 284
        ExplicitLeft = 284
      end
      inherited ButtonToolBarCloseButton: TAdvGlowButton
        Left = 294
        ExplicitLeft = 294
      end
      inherited ButtonToolBarPrintButton: TAdvGlowMenuButton
        Left = 184
        ExplicitLeft = 184
      end
      object ButtonToolBarSaveButton: TAdvGlowButton
        Left = 98
        Top = 2
        Width = 86
        Height = 27
        AutoSize = True
        Caption = #48156#51452'(F5)'
        DisabledPicture.Data = {
          
          44AE426082}
        FocusType = ftHot
        NotesFont.Charset = DEFAULT_CHARSET
        NotesFont.Color = clWindowText
        NotesFont.Height = -11
        NotesFont.Name = 'Tahoma'
        NotesFont.Style = []
        Picture.Data = {
          }
        TabOrder = 3
        OnClick = ButtonToolBarSaveButtonClick
        Appearance.BorderColor = 14727579
        Appearance.BorderColorHot = 10079963
        Appearance.BorderColorDown = 4548219
        Appearance.BorderColorChecked = 4548219
        Appearance.Color = 15653832
        Appearance.ColorTo = 16178633
        Appearance.ColorChecked = 11918331
        Appearance.ColorCheckedTo = 7915518
        Appearance.ColorDisabled = clNone
        Appearance.ColorDisabledTo = clNone
        Appearance.ColorDown = 7778289
        Appearance.ColorDownTo = 4296947
        Appearance.ColorHot = 15465983
        Appearance.ColorHotTo = 11332863
        Appearance.ColorMirror = 15586496
        Appearance.ColorMirrorTo = 16245200
        Appearance.ColorMirrorHot = 5888767
        Appearance.ColorMirrorHotTo = 10807807
        Appearance.ColorMirrorDown = 946929
        Appearance.ColorMirrorDownTo = 5021693
        Appearance.ColorMirrorChecked = 10480637
        Appearance.ColorMirrorCheckedTo = 5682430
        Appearance.ColorMirrorDisabled = clNone
        Appearance.ColorMirrorDisabledTo = clNone
        Appearance.GradientHot = ggVertical
        Appearance.GradientMirrorHot = ggVertical
        Appearance.GradientDown = ggVertical
        Appearance.GradientMirrorDown = ggVertical
        Appearance.GradientChecked = ggVertical
        Appearance.SystemFont = False
        Enabled = False
      end
    end
    inherited ConditionToolBar: TAdvToolBar
      inherited ConditionToolBarFromDateEdit: TcxDateEdit
        Visible = True
      end
      inherited ConditionToolBarDateLabel: TcxLabel
        Caption = #48708#44368#44592#44036
        Visible = True
        AnchorX = 35
        AnchorY = 16
      end
      inherited ConditionToolBarToDateEdit: TcxDateEdit
        Visible = True
      end
      inherited ConditionToolBarDateToLabel: TcxLabel
        Visible = True
        AnchorX = 166
        AnchorY = 16
      end
      inherited ConditionToolBarEditLabel: TcxLabel
        Visible = True
        AnchorX = 448
        AnchorY = 16
      end
      inherited ConditionToolBarComboBox: TcxComboBox
        Properties.Items.Strings = (
          '['#49324#50857#50504#54632']'
          #51204#50900
          #52572#44540' 3'#44060#50900
          #52572#44540' 6'#44060#50900
          #51204#45380' 3'#44060#50900
          #51204#45380' 6'#44060#50900)
        Properties.OnChange = ConditionToolBarComboBoxPropertiesChange
        Text = '['#49324#50857#50504#54632']'
        Visible = True
      end
    end
  end
  inherited Grid: TcxGrid
    Width = 958
    Height = 531
    RootLevelOptions.DetailTabsPosition = dtpTop
    OnActiveTabChanged = GridActiveTabChanged
    ExplicitWidth = 958
    ExplicitHeight = 531
    inherited GridTableView: TcxGridTableView
      OnDblClick = GridTableViewDblClick
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewEXPECT
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewEXPECT5
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewEXPECT10
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewEXPECT15
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewEXPECT20
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewEXPECT25
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableViewEXPECT30
        end>
      DataController.Data = {
        
        7272656E637956616C75655479706500000000}
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      object GridTableViewTrdCode: TcxGridColumn
        Tag = 98
        Caption = #44144#47000#52376#53076#46300
        DataBinding.FieldName = 'CD_TRDPL'
        PropertiesClassName = 'TcxLabelProperties'
        Properties.Alignment.Horz = taCenter
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableViewColumn2: TcxGridColumn
        Tag = 98
        Caption = #44144#47000#52376#51060#47492
        DataBinding.FieldName = 'NM_TRDPL'
        HeaderAlignmentHorz = taCenter
        Width = 200
      end
      object GridTableViewEXPECT: TcxGridColumn
        Tag = 98
        Caption = '4/10'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableViewEXPECT5: TcxGridColumn
        Tag = 98
        Caption = '4/15'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT5'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableViewEXPECT10: TcxGridColumn
        Tag = 98
        Caption = '4/20'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT10'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableViewEXPECT15: TcxGridColumn
        Tag = 98
        Caption = '4/25'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT15'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableViewEXPECT20: TcxGridColumn
        Tag = 98
        Caption = '4/30'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT20'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableViewEXPECT25: TcxGridColumn
        Tag = 98
        Caption = '5/1'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT25'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
      object GridTableViewEXPECT30: TcxGridColumn
        Tag = 98
        Caption = '5/5'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT30'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 80
      end
    end
    object GridTableView1: TcxGridTableView [1]
      Tag = 99
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableView1EXPECT
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableView1EXPECT5
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableView1EXPECT10
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableView1EXPECT15
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableView1EXPECT20
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableView1EXPECT25
        end
        item
          Format = ',0'
          Kind = skSum
          Column = GridTableView1EXPECT30
        end
        item
          Format = ',0'#44148
          Kind = skCount
          Column = GridTableView1GoodsCode
        end>
      DataController.Summary.SummaryGroups = <>
      DataController.OnAfterPost = GridTableView1DataControllerAfterPost
      OptionsBehavior.FocusCellOnTab = True
      OptionsBehavior.FocusFirstCellOnNewRecord = True
      OptionsBehavior.GoToNextCellOnEnter = True
      OptionsBehavior.FocusCellOnCycle = True
      OptionsView.Footer = True
      OptionsView.GroupByBox = False
      Styles.ContentEven = StyleEven
      object GridTableView1GoodsCode: TcxGridColumn
        Caption = #49345#54408#53076#46300
        DataBinding.FieldName = 'CD_GOODS'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 110
      end
      object GridTableView1Column2: TcxGridColumn
        Caption = #49345#54408#51060#47492
        DataBinding.FieldName = 'NM_GOODS'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Focusing = False
        Width = 200
      end
      object GridTableView1Column3: TcxGridColumn
        Caption = #44508#44201
        DataBinding.FieldName = 'NM_SPEC'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 100
      end
      object GridTableView1Column4: TcxGridColumn
        Caption = #51201#51221#51116#44256
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_SAFETY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1Column5: TcxGridColumn
        Caption = #51116#44256#47049
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_STOCK'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1OrderQty: TcxGridColumn
        Caption = #48156#51452#47049
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_ORDER'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        HeaderAlignmentHorz = taCenter
        Options.Filtering = False
        Width = 70
      end
      object GridTableView1Column7: TcxGridColumn
        Caption = #51068#54217#44512#54032#47588#47049
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_DAY'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 100
      end
      object GridTableView1EXPECT: TcxGridColumn
        Caption = '4/10'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1EXPECT5: TcxGridColumn
        Caption = '4/15'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT5'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1EXPECT10: TcxGridColumn
        Caption = '4/20'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT10'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1EXPECT15: TcxGridColumn
        Caption = '4/25'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT15'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1EXPECT20: TcxGridColumn
        Caption = '4/30'
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT20'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1EXPECT25: TcxGridColumn
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT25'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
      object GridTableView1EXPECT30: TcxGridColumn
        DataBinding.ValueType = 'Currency'
        DataBinding.FieldName = 'QTY_EXPECT30'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.Alignment.Horz = taRightJustify
        Properties.DecimalPlaces = 2
        Properties.DisplayFormat = ',0.##'
        FooterAlignmentHorz = taRightJustify
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Focusing = False
        Width = 70
      end
    end
    inherited GridLevel: TcxGridLevel
      Caption = #44144#47000#52376#48324
    end
    object GridLevel1: TcxGridLevel
      Caption = ' '#49345#54408#48324' '
      GridView = GridTableView1
    end
  end
  inherited EditStyleController: TcxEditStyleController
    Style.IsFontAssigned = True
    PixelsPerInch = 96
  end
  inherited StyleRepository: TcxStyleRepository
    PixelsPerInch = 96
  end
  inherited ButtonToolBarPrintButtonImageList: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited dxComponentPrinter: TdxComponentPrinter
    inherited dxComponentPrinter1Link1: TdxGridReportLink
      PrinterPage.PageHeader.CenterTitle.Strings = (
        #54408#51208' '#50696#51221' '#49345#54408' '#51312#54924)
      ReportDocument.CreationDate = 40423.443944039350000000
      OptionsView.Caption = True
      BuiltInReportLink = True
    end
  end
end
