...
{$IFDEF DELPHI12}
  DBPlatform,
{$ENDIF}
  DB, SqlTimSt, Classes, SysUtils, dxCore, dxCoreClasses;
...
type
  ...
  protected
    ...
    function GetBlobPointer(ADataBuffer: TRecordBuffer; AOffSet: Integer): TValueBuffer;
...
function TdxCustomMemData.GetBlobPointer(ADataBuffer: TRecordBuffer; AOffSet: Integer): TValueBuffer;
var
  APtr: TValueBuffer;
begin
  Result := nil;
  ...
  begin
    APtr := ShiftPointer(ADataBuffer, AOffSet * SizeOf(TValueBuffer));
...
var
  ABufPtr, ADataPtr: TValueBuffer;
begin
  ABufPtr := ShiftPointer(ADataBuffer, AOffSet * SizeOf(TValueBuffer));
  ...
  if ASize > 0 then
  begin
    ADataPtr := AllocMem(ASize + SizeOf(TValueBuffer));
...
begin
  ...
  begin
    ...
    begin
      for i := 0 to BlobFieldCount - 1 do
        SetInternalBlobData(GetRecordData(Buffer), i, GetBlobData(TValueBuffer(FBlobList[FCurRec]), i))
    end;
  end;
end;
  ...
  begin
    ...
    begin
      if SupportedFieldType(ADataSet.Fields[I].DataType) and
        (CompareText(ADataSet.Fields[I].FieldName, 'RECID') <> 0) and
